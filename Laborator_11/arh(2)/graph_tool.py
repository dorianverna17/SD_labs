...
def visualize(args):
    df = pd.DataFrame.from_csv(args.path)
    ...
    nx.draw_networkx_edges(G, pos,  width=5.0, edge_color='b', arrows=True)
    plt.show()
...
def generate(args):
    ...
    while(len(edges) < min(N * (N - 1) / 2,  args.edges)):
        ...
        #if from_id == to_id or in_link_count[to_id] > out_link_count[from_id] + 1:
        #    continue
